...
## Backend: Package Restructuring by Feature

Issue #1's wayfinder map flagged this directly, while resolving [Frontend project structure & conventions](https://github.com/SolonKarandanis/event-ticketing-platform/issues/2), as a future idea rather than something to build as part of that map: `ticket-service`'s package layout was organized by technical layer (`controller/`, `domain/entity/`, `domain/service/`, `domain/dto/{request,response}/`, `domain/repository/`, `domain/enums/`, `domain/exception/`, `domain/model/`, `domain/event/`), while the frontend -- built afterward, from scratch -- was organized by feature (`src/features/<entity>/{api,types,hooks,components}`). Picked up after the map closed, the same way PostGIS, ticket cancellation, and event images were: a **behavior-preserving** reorganization, not a rewrite. Every class kept its name, its content, its public contract, and its Spring annotations -- only its package, and therefore its import references elsewhere, changed.

### One Package Per Domain Concept, Not Per Layer

`@SpringBootApplication` uses default component scanning with no explicit `basePackages`, and there's no ArchUnit/checkstyle/package-info enforcement anywhere in the build, so sub-packages were free to reorganize with no framework wiring to touch. The result is one package per feature, each holding its own controller, entity, repository, service, and `dto`/`model`/`exception` subpackages: `events` (with a nested `images` sub-feature), `venues`, `tickettypes`, `tickets` (with a nested `qrcode` sub-feature), `ticketvalidation`, and `user`. `config` and `security` stay exactly where they were. A new `messaging` groups the cross-feature eventing pieces (`TicketEventPublisher`, `RabbitMqTicketEventListener`, `TicketPurchasedEvent`, `TicketCancelledEvent`) since three different features publish through it and no single one can own it. A new `common` package -- the backend's analogue of the frontend's `src/lib/` -- holds the rest of what has no single feature owner: the `ErrorCode`/`EventTicketException`/`GlobalExceptionHandler` exception subsystem, the `AbstractPaging`/`Paging`/`SearchRequestDTO` paging framework, `MultipartFileUtil`, and the antivirus abstraction (used only by event-image uploads today, but generic and infrastructure-shaped, not a domain feature in its own right).

Two placement calls worth calling out: published events fold into `events` rather than getting their own package, since `PublishedEventController` calls straight into the same `EventService`/`Event` as the organizer-facing side -- there's no separate service or entity for it on the backend the way the frontend's separate `features/published-events` might suggest, and splitting further would mean decomposing `EventService` itself, a behavior change well outside a package reorg. And `images`/`qrcode` nest under `events`/`tickets` respectively rather than becoming their own top-level packages, since both are child concepts owned by exactly one parent (`EventImage.event`, `QrCode.ticket`) and never referenced from anywhere else -- the same "promote to a shared location only on a second real use" discipline the frontend already applies to its own `src/components/`.

### Where Implicit Same-Package Resolution Broke

Most of the ~160 files needed nothing but a package-declaration and import-path rewrite -- `controller`/`domain.service`/`domain.entity`/`domain.dto.*`/`domain.exception` were already separate packages from each other, so cross-references between them already went through explicit imports. Two places didn't get off that easily, because they relied on **implicit same-package resolution** that breaks the moment siblings land in different feature packages:

- **Entity classes.** `Event.java` referenced `Venue`, `TicketType`, `User`, and `EventImage` with zero imports, because all five entities lived in one flat `domain.entity` package. Once they split across `events`/`venues`/`tickettypes`/`user`/`events.images`, every one of those references needed an explicit import added. Same story for `Ticket.java` (→ `TicketType`, `User`, `TicketValidation`, `QrCode`), `TicketType.java` (→ `Event`, `Ticket`), and the rest of the entity graph.
- **Exception subclasses.** Every subclass of `EventTicketException` (`EventNotFoundException`, `VenueNotFoundException`, `TicketsSoldOutException`, and around twenty others) referenced `ErrorCode` and `EventTicketException` itself with zero imports, because the whole hierarchy lived in one flat `domain.exception` package. Redistributing the subclasses into their owning feature's `exception` subpackage while `ErrorCode`/`EventTicketException` moved to `common.exception` meant every subclass needed both imports added.

Both surfaced as `cannot find symbol` the instant the packages split -- the compiler is the safety net for exactly this class of change. The move itself (`git mv` plus a rewritten `package` line, driven by a scripted old-FQCN-to-new-FQCN mapping) was scripted; so was the repo-wide `import` rewrite for every relocated class. What the compiler caught -- and a script fixed, reading each `cannot find symbol` error and inserting the missing import from a simple-name index built off the new tree -- was resolved through a short iterative compile-fix-recompile loop rather than trying to predict every implicit reference by hand up front. A handful of stray cases (a wildcard `import ...antivirus.*;` and two nested-class imports the mechanical rewrite couldn't pattern-match, plus `import static ...TestJwts.withSubject;` in eight test files) needed a manual pass on top. A final cleanup pass stripped 116 imports that had become same-package-redundant once a class's former neighbors joined it in the same new feature package, and collapsed a few stray blank lines the import-insertion left behind.

### Verification

`./gradlew compileJava compileTestJava` clean, then the full test suite (`@WebMvcTest` controller slices, Testcontainers Postgres/PostGIS `@DataJpaTest` repository tests, service-layer unit tests) run twice -- once before the redundant-import/blank-line cleanup, once after -- both times **145/145 tests passing, 0 failures, 0 errors**. `git diff --stat` confirms the change is scoped to file moves plus package/import edits: 154 files changed, 652 insertions, 643 deletions, no `.java` file outside an import block or its `package` line touched.

#### Summary

- Reorganized `ticket-service` from layer-based packages (`controller/`, `domain/{entity,service,dto,repository,enums,exception,model,event}/`) to feature-based ones (`events` + nested `images`, `venues`, `tickettypes`, `tickets` + nested `qrcode`, `ticketvalidation`, `user`), plus cross-cutting `common` (exception subsystem, paging framework, util, antivirus) and `messaging` -- `config`/`security` untouched
- Purely mechanical and behavior-preserving: every class kept its name, content, and annotations; only its package and import references changed
- The two places implicit same-package resolution broke -- entity cross-references and exception subclasses' `ErrorCode`/`EventTicketException` -- were both caught by the compiler and fixed through an iterative compile-fix loop, not predicted by hand up front
- Verified via a clean compile plus the full 145-test suite, run twice, unchanged pass/fail both times
- Originally flagged as a future idea in issue #1's Out of scope list while resolving [Frontend project structure & conventions](https://github.com/SolonKarandanis/event-ticketing-platform/issues/2); picked up after the map closed, the same way PostGIS, ticket cancellation, and event images were
...
`ticket-service`'s package-by-feature refactor (also originally out of scope, flagged while resolving issue #2) is done too -- see "Backend: Package Restructuring by Feature" above: the layer-based `controller/`/`domain/{entity,service,dto,repository,enums,exception,model,event}/` layout is now one package per domain concept (`events` + nested `images`, `venues`, `tickettypes`, `tickets` + nested `qrcode`, `ticketvalidation`, `user`, plus cross-cutting `common`/`messaging`), matching the frontend's own feature-folder convention. Purely mechanical -- no class's name, content, or behavior changed, only its package -- and verified by a clean compile plus the full 145-test suite passing unchanged before and after.

Automated testing (also originally out of scope) is now built too, on both sides of the stack. `ticket-service` has `@WebMvcTest` slice tests for every controller (`EventControllerTest`, `PublishedEventControllerTest`, `TicketControllerTest`, `TicketTypeControllerTest`, `TicketValidationControllerTest`, `VenueControllerTest`) plus a dedicated `SecurityAuthorizationTest` and `JwtAuthenticationConverterTest`; `@DataJpaTest` slice tests for every repository, run against a real Testcontainers Postgres container rather than H2 (`AbstractPostgresContainerTest`, pinned to `postgis/postgis:14-3.5` to match the live dev database, since `EventRepository`/`VenueRepository`'s native geo queries need PostGIS to exist at all); and unit tests for every `*ServiceImpl`. `frontend` has React Query hook tests for `events`, `published-events`, `ticket-sales`, `tickets`, and `venues`, backed by MSW request mocking rather than a real backend. `analytics-service` still has none. Nothing here is wired into CI yet (no `.github/workflows`) -- "automated" means "a real test suite exists and passes," not "runs automatically on every push."

### `frontend` loose ends

- React Query hook tests exist for `events`, `published-events`, `ticket-sales`, `tickets`, and `venues` (MSW-backed, see "Project Status" above) -- no component/route tests and no e2e tests yet, and none of it runs in CI
...
- `@WebMvcTest` controller slice tests, `@DataJpaTest` repository slice tests (against a real Testcontainers Postgres/PostGIS container, not H2), and service-layer unit tests now exist for every controller/repository/service -- see "Project Status" above. Not yet wired into CI; still verified locally via `bootRun` plus `psql`/`curl`/browser checks on top of the test suite
...
- The package-by-feature refactor (see "Backend: Package Restructuring by Feature") moved every class into a new package -- no endpoint, entity, or behavior changed. Confirmed via `./gradlew compileJava compileTestJava` (clean) and `./gradlew test` (145/145 passing, 0 failures, run twice -- once before and once after a follow-up cleanup pass that stripped 116 imports that became same-package-redundant), not yet re-confirmed against a live `bootRun`/browser click-through, though none was expected to be needed for a package-only move
